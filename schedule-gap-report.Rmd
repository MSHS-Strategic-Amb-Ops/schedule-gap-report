...
held_hours <- availability_raw_tbl %>%
  ...
  mutate(Appt.DateYear = TO_DATE(SLOT_DATE)) %>%
  group_by(DEPARTMENT_NAME, DEPARTMENT_ID, PROV_NM_WID, PROV_ID, Appt.Year, Appt.Month, Appt.DateYear, SLOT_BEGIN_TIME, SLOT_LENGTH, TIME_HELD_RSN_NAME) %>%
  summarise(avail_min = n()) %>%
  # ungroup() %>%
  # group_by(DEPARTMENT_NAME, DEPARTMENT_ID, PROV_NM_WID, PROV_ID, Appt.Year, Appt.Month, Appt.DateYear) %>%
  # summarise(avail_min = sum(SLOT_LENGTH)) %>%
...
held_hours <- held_hours %>%
  ungroup() %>%
  mutate(Appt.DateYear = as.Date(Appt.DateYear, "%Y-%m-%d")) %>%
  group_by(DEPARTMENT_NAME, DEPARTMENT_ID, PROV_NM_WID, PROV_ID, Appt.Year, Appt.Month, Appt.DateYear, TIME_HELD_RSN_NAME) %>%
...
```{r Gap Analysis, echo = FALSE, warning = FALSE, message = FALSE}

# outpatient_cFTE <- read_excel("outpatient_cFTE_Q1 2023.xlsx")
# 
# 
# hours_summary <- utilization_output %>%
#   group_by(PROV_NAME, NPI, VISIT_PROV_STAFF_RESOURCE_C, Appt.MonthYear) %>%
#   summarise(avail_min = sum(avail_min, na.rm = T),
#             sched_min = sum(sched_min , na.rm = T),
#             arrived_min = sum(arrived_min, na.rm = T)) %>%
#   mutate(schedule_util = round(sched_min/avail_min,2),
#          provider_util = round(arrived_min/avail_min,2)) %>%
#   mutate(NPI = as.numeric(NPI))
# 
# 
# gap_analysis <- left_join(hours_summary, outpatient_cFTE, by = c("NPI"))
# 
# 
# gap_analysis <- gap_analysis %>%
#   mutate(expected_min = (40*Outpatient)*60,
#          expected_avail_min_variance = expected_min - avail_min,
#          expected_avail_hour_variance = round(expected_avail_min_variance/60,1)) 
# 
# gap_col_order <- c("Master Dept", "Site", "Physician", "NPI", "Total FTE", "2023 Reported CFTE", 
#                    "Outpatient Practice Locations", "Comments / Job Description", 
#                    "Outpatient", "OR/Procedure","Inpatient","Other",
#                    "avail_hour", "sched_min", "arrived_min", "expected_min", "expected_avail_min_variance", "expected_avail_hour_variance",
#                    "schedule_util", "provider_util")
# 
# gap_analysis <- gap_analysis[,gap_col_order]
